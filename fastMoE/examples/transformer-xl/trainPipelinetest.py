...
print("let check which version")
corpus = get_lm_corpus(args.data, args.dataset)
# corpus, tr_iter, va_iter, te_iter, ntokens = get_lm_corpus_subset(args.data, args.dataset)
...
def trace_function_calls(frame, event, arg):
    if event == 'call':
        ...
        # Define the path segment that identifies your package
        target_package_path = 'site-packages/fastmoe/'

        # Check if the current file belongs to the target package
        if target_package_path in filename:
            print(f"Called {func_name} in {filename} on line {lineno}")
        else:
            return None  # Skip further tracing in this call if it's outside the target package
...
def train():
    ...
    for batch, (data, target, seq_len) in enumerate(train_iter):
        ...
        if train_step % args.log_interval == 0:
            ### TODO DELETE IF TRAINING REQUIRED
