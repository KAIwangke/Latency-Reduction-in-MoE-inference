...
class RelMultiHeadAttn(nn.Module):
    def __init__(self, n_head, d_model, d_head, dropout, dropatt=0,
        ...
        self.scale = 1 / (d_head ** 0.5)
        print("the number of the experts : ",moe_num_expert)
...
class CustomizedMoEPositionwiseFF(FMoETransformerMLP):
    ...
    def forward(self, inp, layer_idx=None):
        if self.pre_lnorm:
            ##### layer normalization + positionwise feed-forward
            core_out = super().forward(self.layer_norm(inp), layer_idx=layer_idx)
            ...
            ##### positionwise feed-forward
            core_out = super().forward(inp, layer_idx=layer_idx)
            core_out = self.dropout(core_out)
...
class RelPartialLearnableDecoderLayer(nn.Module):
    ...
    def forward(self, dec_inp, r, r_w_bias, r_r_bias, dec_attn_mask=None, mems=None, layer_idx=None):
        ...
        output = self.pos_ff(output, layer_idx=layer_idx)
    ...
    def _forward(self, dec_inp, mems=None):
        ...
        if self.attn_type == 0: # default
            ...
            for i, layer in enumerate(self.layers):
                ...
                chosen_experts = torch.argmax(core_out, dim=-1)
